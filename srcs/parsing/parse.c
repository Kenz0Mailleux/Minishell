...
/*   Updated: 2024/09/30 18:05:45 by kmailleu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	handle_pipe(t_cmd **current_cmd, t_cmd **cmd_head, int *num_cmd)
{
	(*num_cmd)++;
...
static void	handle_cmd_token(t_cmd *current_cmd, t_token *current_token)
{
	current_cmd->tab_cmd = ft_join_tab(
		current_cmd->tab_cmd, current_token->str, current_cmd->tab_len);
	(current_cmd->tab_len)++;
}

static int	handle_redirect_token(t_cmd *current_cmd, t_token **current_token, t_redirect **red_head)
{
	if ((*current_token)->next && (*current_token)->next->type == CMD)
	{
		append_redirect(red_head,
			create_redirect((*current_token)->type, (*current_token)->next->str));
...
static void	process_tokens(t_cmd **current_cmd, t_cmd **cmd_head, t_token **current_token)
{
	int			num_cmd;
	t_redirect	*red_head;
	...
	while (*current_token != NULL)
	{
		if ((*current_token)->type == PIPE
			...
			handle_pipe(current_cmd, cmd_head, &num_cmd);
		else if ((*current_token)->type == CMD)
			handle_cmd_token(*current_cmd, *current_token);
		else if ((*current_token)->type > 1 && (*current_token)->type < 7)
		{
			if (!handle_redirect_token(*current_cmd, current_token, &red_head))
				return;
		}
		*current_token = (*current_token)->next;
	}
...
t_cmd	*parser(t_data *data)
{
	...
	cmd_head = NULL;
	append_cmd(&cmd_head, create_cmd(0));
	current_cmd = cmd_head;
	current_token = data->token;
	process_tokens(&current_cmd, &cmd_head, &current_token);
	data->cmd = cmd_head;
	return (cmd_head);
}
